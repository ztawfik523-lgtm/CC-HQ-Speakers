# Documentation index

This fork uses a lighter version of the useful HighAudio docs structure.

| Need | File |
|---|---|
| What exists now? | `CURRENT-STATE.md` |
| What do we build next? | `ROADMAP.md` |
| How does it work? | `ARCHITECTURE.md` |
| What is proven? | `VERIFIED-FACTS.md` |
| What are the known problems? | `KNOWN-ISSUES.md` |
| How do we test? | `TESTING.md` |
| How do we run the M0 runtime baseline? | `M0-SMOKE-TEST.md` |
| Where did facts/dependencies come from? | `SOURCES.md` |
| What HighAudio work transfers? | `research/HIGHAUDIO-TRANSFERABLE-FINDINGS.md` |
| What SPR work already exists? | `research/SPR-INTEGRATION-BASELINE.md` |

The goal is not ceremony. It is to stop future agents from rediscovering the same facts or rebuilding working systems unnecessarily.
